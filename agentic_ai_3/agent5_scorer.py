...
# Two camera FOVs should not cover the same ground — every new camera
# should expand coverage, not duplicate an existing one. 0 = strict "must
# not touch"; a small positive number tolerates rasterization noise at
# cone edges without treating it as real overlap.
OVERLAP_TOLERANCE_CELLS = 0
...
# ---- Hard constraint: cameras must not overlap or sit in each other's FOV
# Two things disqualify a candidate:
#   1. Its FOV cone shares more than OVERLAP_TOLERANCE_CELLS cells with any
#      already-placed camera's FOV (two cones watching the same ground).
#   2. Its own base position falls inside an already-placed camera's FOV
#      (one camera literally sitting inside another's cone).
# OVERLAP_TOLERANCE_CELLS allows a small amount of edge overlap (cone
# boundaries are pixel-rasterized, so a handful of shared border cells is a
# rasterization artifact, not real redundancy); set to 0 for a completely
# strict "must not touch" rule.
for cand in candidates:
    cand_fov = frozenset(map(tuple, cand.get("fov_cells", [])))
    base_cell = (int(round(cand["row"])), int(round(cand["col"])))
    cand["overlap_cells"]    = len(cand_fov & covered_cells_so_far)
    cand["base_in_existing_fov"] = base_cell in covered_cells_so_far

non_overlapping = [
    c for c in candidates
    if c["overlap_cells"] <= OVERLAP_TOLERANCE_CELLS and not c["base_in_existing_fov"]
]

if non_overlapping:
    candidates = non_overlapping
    candidates.sort(key=lambda c: c["final_score"], reverse=True)
    print(f"{len(non_overlapping)} candidate(s) satisfy the no-overlap "
          f"constraint (<= {OVERLAP_TOLERANCE_CELLS} shared cells, base not "
          f"inside any existing FOV).")
    best = candidates[0]
else:
    # No candidate is overlap-free. Previously this force-placed the
    # least-overlapping candidate anyway, which is exactly the overlap
    # you don't want — so instead: place NOTHING this iteration.
    print("\n[SKIP] No candidate satisfies the no-overlap constraint this "
          "iteration — every shortlisted position either overlaps an "
          "existing camera's FOV or its own base sits inside one. "
          "Not placing a camera rather than force an overlap.")
    print("       Consider: widening Agent 3's candidate pool "
          "(CANDIDATES_PER_BN / CANDIDATES_PER_BLIND_GRID), enlarging the "
          "search radius, or reducing FOV_DEG/MAX_RANGE if the site is "
          "small relative to camera range.")

    # Routes pass through unchanged (no camera placed => no elimination),
    # but the iteration counter still advances so Agent 3 reshortlists
    # fresh candidates next time instead of the pipeline repeating the
    # exact same (failed) iteration forever.
    ...
    skip_result = {
        ...
        "placed"            : False,
        "reason"            : "no candidate satisfies the no-overlap constraint",
    ...
    print(f"\nWrote pass-through routes -> paths_iter_{next_iter}.json")
    print("AGENT 5 COMPLETE (no camera placed this iteration)")
    sys.exit(0)

print(f"\nBest candidate id={best['candidate_id']}  overlap_cells={best['overlap_cells']}")
...
new_entry = {
    ...
    "overlap_cells"      : best.get("overlap_cells"),
